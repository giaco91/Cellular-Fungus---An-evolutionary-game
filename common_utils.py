...
def create_new_image(height,width):
    return create_image(height,width)
...
def npimg_to_int_array(np_img):
    eps=0.15#smaller than 1.5
    s=np_img.shape
    int_img=np.zeros((s[1],s[2])).astype(int)
    for i in range(s[1]):
        for j in range(s[2]):
            if np.abs(np_img[0,i,j]-np_img[1,i,j])<eps and np.abs(np_img[0,i,j]-np_img[2,i,j])<eps:
                S=np.sum(np_img[:,i,j])
                if S>1.5:
                    int_img[i,j]=0
                elif S<1.5:
                ...
                int_img[i,j]=2+np.argmax(np_img[:,i,j])
    return int_img

def int_array_to_npimg(int_img):
    s=int_img.shape
    np_img=np.zeros((3,s[0],s[1])).astype(int)
    for i in range(s[0]):
        for j in range(s[1]):
            if int_img[i,j]==0:
                np_img[0,i,j]=np_img[1,i,j]=np_img[2,i,j]=1
            elif int_img[i,j]==2:
                np_img[0,i,j]=1
            elif int_img[i,j]==3:
                np_img[1,i,j]=1
